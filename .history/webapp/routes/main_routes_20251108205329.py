...
def predict():
    ...
    if file and allowed_file(file.filename):
        ...
        if not os.path.exists(model_path):
            return "❌ Trained model not found! Run training first.", 500
        ...
        emb = extractor.extract_features(save_path).reshape(1, -1)
        ...
        cuisines = cuisines_map.get(pred_label, ["Local Food"])

        # ✅ Get Metrics from app (loaded in app.py)
    ...
    return "File not allowed, only .wav expected.", 400
